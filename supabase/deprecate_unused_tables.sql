...
-- plus `profiles`, which nothing in the frontend queries):
--   habits, habit_logs, goals, goal_milestones, goal_habit_relations,
--   journal_entries, flashcards, achievements, user_achievements,
--   tasks, categories, calendar_events, profiles

-- OPTIONAL preflight — run this first to check whether any Postgres
-- function/trigger (e.g. an on-signup handler writing to `profiles`)
...
-- where prosrc ~* 'habits|habit_logs|goals|goal_milestones|goal_habit_relations|journal_entries|flashcards|achievements|user_achievements|tasks|categories|calendar_events|profiles';
...
ALTER TABLE profiles             RENAME TO _deprecated_profiles;
