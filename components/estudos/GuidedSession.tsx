...
export function GuidedSession({ subjects, topics, subjectId, activeDay, onToggle }: Props) {
  ...
  // group by subject when showing all
    ...
    <div className="card" style={{ padding: 0, overflow: 'hidden', marginTop: 16 }}>
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--bdr)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: '#121826' }}>
        ...
        <div style={{ height: 4, background: '#E9ECF2', borderRadius: 99 }}>
          <motion.div
            style={{
      ...
      <div>
        {groups.map((group, gi) => (
          <div key={group.subject?.id ?? gi}>
            {/* Subject label when showing multiple */}
